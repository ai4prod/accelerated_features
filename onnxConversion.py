...
"""
1) python ./onnxConversion.py --dynamic  --export_path ./xfeat_matching.onnx FUNZIONA PER TENSORRT
Verificare che dynamic_export= True sia attivo per avere un modello dinamico

2) Una ottenuto il modello eseguire  python3.8 shape_inference.py --input xfeat_matching.onnx --output xfeat_matching_tensorrt.onnx


python ./onnxConversion.py  --export_path ./xfeat_matching.onnx Non funziona per tensorRT
...
if __name__ == "__main__":
    ...
    if args.xfeat_only_model_detectAndCompute:
        ...
        if dynamic_export:
            ...
            x1 = torch.randn(batch_size, 3, args.height, args.width, dtype=torch.float32, device='cpu')
            x2 = torch.randn(batch_size, 3, args.height, args.width, dtype=torch.float32, device='cpu')
            print("EXPORT FIXED STAR")
            print("MODEL INPUT SIZE ",x2.size())
            dynamic_axes = {"matches": {0: "batch", 1: "keypoints"}, "batch_indexes": {0: "values",}}
            torch.onnx.export(
                xfeat,
                (x1, x2),
                args.export_path,
